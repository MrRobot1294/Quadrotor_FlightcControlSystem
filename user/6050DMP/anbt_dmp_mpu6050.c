...
u8 AnBT_DMP_MPU6050_DEV_CFG(void)		
{
	  ...
	  if (anbt_dmp_data[0]!=PRODUCT_WHOAMI) 
		{
			//AnBT_Uart1_Send_String("I2C从机地址设置错误，请将AD0端口接地",36);
...
{
	u8 i;                   //为了不影响if语句的实施，设置i空加
	AnBT_DMP_MPU6050_DEV_CFG();		//etootle: init the mpu6050 chip				
	//AnBT_Uart1_Send_String("1.DMP设备初始化成功",19); 		//完成初始化
	//
	if(!mpu_set_sensors(INV_XYZ_GYRO | INV_XYZ_ACCEL))i++; //AnBT_Uart1_Send_String("2.DMP设置传感器成功",19);		//完成传感去设定
	//else// AnBT_Uart1_Send_String("2.DMP设置传感器错误",19);
	//
	if(!mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL))i++; //AnBT_Uart1_Send_String("3.设置FIFO成功",14);		//完成fifo设置
	//else //AnBT_Uart1_Send_String("3.设置FIFO失败",14);
	//
	if(!mpu_set_sample_rate(DEFAULT_MPU_HZ))i++;//AnBT_Uart1_Send_String("4.DMP设置采样率成功",19);			//完成DMP采样率设置
	//else //AnBT_Uart1_Send_String("4.DMP设置采样率失败",19);
	//
	if(!dmp_load_motion_driver_firmware())i++;	//AnBT_Uart1_Send_String("5.DMP加载运动驱动固件成功",25);				//开启DMP功能
	//else //AnBT_Uart1_Send_String("5.DMP加载运动驱动固件失败",25);
	//
	if(!dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation))) i++;//AnBT_Uart1_Send_String("6.DMP完成设置初始方向设定偏差",29);		//初始方向设定偏差
	//else //AnBT_Uart1_Send_String("6.DMP设置初始方向设定偏差出错",29);
	//
	if(!dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT | DMP_FEATURE_TAP |
	DMP_FEATURE_ANDROID_ORIENT | DMP_FEATURE_SEND_RAW_ACCEL | DMP_FEATURE_SEND_CAL_GYRO |
	DMP_FEATURE_GYRO_CAL))	i++;	//AnBT_Uart1_Send_String("7.DMP使能成功",13);		//DMP使能
	//else AnBT_Uart1_Send_String("7.DMP使能出错",13);
	//
	if(!dmp_set_fifo_rate(DEFAULT_MPU_HZ)) i++;//AnBT_Uart1_Send_String("8.DMP设置FIFO速率成功",21);		//DMP  fifo速度
	//else AnBT_Uart1_Send_String("8.DMP设置FIFO速率失败",21);
	//
	run_self_test();
	if(!mpu_set_dmp_state(1))i++;// AnBT_Uart1_Send_String("9.Dmp启动成功",13);				//DMP成功开启
	//else AnBT_Uart1_Send_String("9.Dmp启动失败",13);					//Dmp启动失败
	//
	return 0;
}
...
//dmp的延时函数
 void DMP_Delay( uint32_t nCount )
{
  for ( ; nCount != 0; nCount -- );	
}
